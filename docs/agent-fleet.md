...
| 阶段 3 收尾 | ✅ 完成 |
| 阶段 4 可选增强 | 🔶 E1/E2/E3/E5/E6 完成；E7/E8 待触发，E4 未开始 |
...
原计划里 E6 写的是"性能优化，等面板变迟钝再做"。实际做下来，**性能一点没提，全部工作量都花在正确性上**——因为它要保护的东西（文本选区）有一条很硬的物理约束：

> **被 `insertBefore` 移动的节点会被重新挂载，落在它上面的选区当场消失。**
> 同理，给 `textContent` 赋值——**哪怕赋的是同一个字符串**——也会换掉底下的文本节点。

所以"这一轮搬哪些节点"不是性能问题，是正确性问题。**只要多搬了一个不该搬的节点，E6 就白做了**，而且失败是静默的：界面看起来完全正常，只有正在拖蓝的用户会发现选区没了。

实现分三层，每层都是被 review 打回来才补上的：

1. **`setText` 只在文本真变了时才写**。少了这个判断，keyed 更新等于没做。
2. **加权 LIS 决定谁原地不动**（`pickStayPut`，模块级导出的纯函数）。最初写的是一趟贪心 cursor（"cursor 不等于当前节点就 insertBefore"），它有两个隐蔽的失败模式：挡路的节点是**这一轮要被删的**、或者挡路的节点**活着但该往后排**。两种情况下贪心都会去搬后面那张无辜的卡片。
3. **选区落在谁身上，谁的权重压倒一切**（`SELECTED_WEIGHT`）。同组两张卡片换位时两边都是卡片，权重一样，DP 的严格大于比较会稳定地拍向先遇到的那支——而重排里"往上跑"的恰恰是数据变了的那张，**没变的那张被往下挤 = 被搬走**。

**三轮 review 各抓出一个真 bug，每一个都是"我以为已经修好了"之后才发现的**：

| 轮次 | 症状 | 触发条件 |
|---|---|---|
| 1 | 前面的会话一退出，它后面所有卡片的选区遭殃 | 任意会话退出 |
| 2 | 旁观者卡片（什么都没发生的那些）被搬 | 换组 / 组内重排 / 整组消失 |
| 3 | 同组两张卡换位时，没变的那张被搬 | 活跃会话 mtime 每轮都在动，**最高频** |

**教训：这类缺陷靠"构造一个场景测一下"抓不完**。前两轮我每次都写了针对性测试并通过，但每次构造的都是"被改动的那张卡"，而咬人的一直是**旁边那些什么都没发生的卡片**。第三轮加了 fuzz（随机多轮增删改，固定 seed）才把面覆盖住——它一次跑 300 轮，比手写十条用例管用。数字对比很能说明问题：去掉选区权重后，同样 242 个采样丢 21 次（8.7%），而那 8.7% 手写用例一条都没碰到。

**几条实测得来、不写下来会被改回去的结论**：

- **textarea 的选区在 document 层面报告 `isCollapsed: true`**（真 Chromium 实测，哪怕真选中了一段）。所以 `currentSelectionNode()` 里那个 `isCollapsed` 判断不只是"没选中就别管"，它恰好挡住了编写区——否则每轮都会把 textarea 误判成需要保护的节点。**这条 jsdom 测不出来**。
- **`pickStayPut` 必须是可单测的纯函数**。第一版把"打平时牺牲标题"写成走真实 report 的 DOM 层测试，那条测试是**假绿**——把 `CARD_WEIGHT` 改成 1 它照样通过，因为那个场景根本没打平。穷举后才知道打平的最简形态是"相邻两项交换且后者是卡片"（oldIndex 形如 `[0,1,2,4,3]`），这在 DOM 层很难拧出来。
- **`SELECTED_WEIGHT` 的量级边界是 100 张卡**（10000 / CARD_WEIGHT）。超了只是退化成"选区丢失"，顺序正确性不依赖权重，不会错乱不会崩。
- **`scrollTop` 的手工存取删掉了**：列表容器不再被重建，浏览器原生保持滚动位置。对应测试的断言从"必须是全新节点"反转成"必须是同一个节点"。
- **清理残留节点要按"这一轮摆过没有"判，不能按节点类型判**。按类型判（`Number(el.dataset.groupIndex) >= n`）会给将来加第三种子节点留个静默失效的口子——`Number(undefined) >= n` 恒为 false，那种节点会既不被清理也不参与摆放，永久滞留。

**没做的**：子 agent 树和警告区仍是整块重建。两者默认收起、展开态各自单独记着，而 E6 的症状发生在卡片主体。真要在树里选文字再说。
