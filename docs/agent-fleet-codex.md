...
1. **`session_meta` 不止一条**。样本里出现在 idx 0 和 idx 31（第二条多了
   `memory_mode`）。首行解析没问题，但不能写「文件里只有一条」的断言。
...
**文件名自带 session_id 和起始时间**（`rollout-2026-08-03T12-23-32-<uuid>.jsonl`），
所以「这个会话是谁、什么时候起的」不用打开文件就知道。这比 Claude 侧要遍历
`projects/` 找 jsonl 便宜得多。
